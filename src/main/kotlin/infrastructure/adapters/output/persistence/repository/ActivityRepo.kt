...
class ActivityRepo() : ActivityRepository {
    ...
    override fun findAll(): List<Activity> {
        return transaction {
            activitiesTable.selectAll().map { row ->
                val id = row[activitiesTable.id].value
                val content = ActivityContents.select { ActivityContents.activityId eq id }
                    .map { contentRow ->
                        ContentItem(
                            id = contentRow[ActivityContents.id].value,
                            texto = contentRow[ActivityContents.texto],
    ...
    override fun findByTeacherId(teacherId: UUID): List<Activity> {
        return transaction {
            activitiesTable.select { activitiesTable.teacherId eq teacherId }.map { row ->
                val id = row[activitiesTable.id].value
                val content = ActivityContents.select { ActivityContents.activityId eq id }
                    .map { contentRow ->
                        ContentItem(
